...
class QLearningFuncApprox:
    def __init__(self, 
        ...
        Args:
            ...
            alpha (float): Learning rate \alpha_t = \alpha > 0.
    ...
    def _get_q_value(self, state: int, action: int, w: np.ndarray) -> float:
        """Responds with q(state, a) = <features, w>"""
    ...
    def _sample_epsilon_greedy_action(self, state: int, w: np.ndarray) -> int:
        """
        Samples an action following the \pi_t(s) policy rule.
    ...
    def __call__(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        while k < self.max_episodes:
            ...
            for _ in range(self.max_steps_per_episode):
                # 1. Generate a_t at s_t following \pi_t(s_t)
                a_t = self._sample_epsilon_greedy_action(s_t, w)
                
                # 2. Interact with the environment to collect r_{t+1}, s_{t+1}
                ...
                # Compute current state-action value estimation
                q_current = self._get_q_value(s_t, a_t, w)
                
                # Q-learning target tracks the maximum value over all actions in the next state: max_{a} q(s_{t+1}, a, w_t)
                q_max_next = 0.0 if terminated else max([self._get_q_value(s_tp1, a, w) for a in range(self.num_actions)])
                
                # Gradient with respect to w is equal to the feature vector array
                grad_q = self._get_features(s_t, a_t)
                
                # Update q-value weights via the Q-learning function approximation correction target rule:
                # w_{t+1} = w_t + \alpha_t * [r_{t+1} + \gamma * max_a q(s_{t+1}, a, w_t) - q(s_t, a_t, w_t)] * \nabla_w q(s_t, a_t, w_t)
                td_error = float(r_tp1) + (self.gamma * q_max_next) - q_current
        ...
        print(f"Q-Learning Function Approximation optimization finished after {k} episodes.")
