...
class REINFORCE:
    ...
    def __call__(self) -> np.ndarray:
        ...
        while k < self.max_episodes:
            # Receptacles for the Monte Carlo episode trajectory
            saved_log_probs = []
            rewards = []
            ...
            for _ in range(self.max_steps_per_episode):
                # Calculate probability distribution for current state
                ...
                # Store logarithmic probability and reward for the objective function
            ...
            # 2. Value Estimation and Policy Update (processed backward through the trajectory)
            returns = []
            G = 0.0
            
            # Calculate q_t(s_t, a_t) = \sum \gamma^{k-t-1} r_k
            ...
            # Formulate the objective loss function: -\ln \pi(a_t|s_t) * q_t
            # (Negative sign because PyTorch optimizers minimize by default, and we want to MAXIMIZE returns)
            policy_loss = []
        ...
        print(f"REINFORCE optimization finished after {k} episodes.")
